...
def load_names(path, file_path):
    f = open(file_path, "r")
    data = f.read().split("\n")[:-1]
    images = [os.path.join(path,"test", "frontal", name) + ".jpg" for name in data]
    masks = [os.path.join(path,"test", "frontal-mask", name) + ".jpg" for name in data]
    return images, masks
...
def load_data(path):
    ...
    valid_x, valid_y = load_names_test(path, valid_names_path)
...
if __name__ == "__main__":
    ...
    path = "/Users/sepehrnouri/CAPSTONE/Data/kavanati-resized/"
