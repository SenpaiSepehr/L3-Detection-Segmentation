...
class Bottleneck(nn.Module):
    def __init__(self, in_c, out_c, dim, num_layers=1):
        super().__init__()
...
class TResUnet(nn.Module):
    def __init__(self):
        ...
        self.b1 = Bottleneck(256, 64, 256, num_layers=1)
        ...
        """ Decoder ResNet18 """
        self.d1 = DecoderBlock([64, 128], 128)  # original: 256
    ...
    def forward(self, x, heatmap=None):
        s0 = x                  ## [-1, 3, h, w]
        s1 = self.layer0(s0)    ## [-1, 64, h/2, w/2]        resnet18: [-1,64, h/2, h/2]
        s2 = self.layer1(s1)    ## [-1, 256, h/4, w/4]       resnet18: [-1,64, h/4, h/4]
        s3 = self.layer2(s2)    ## [-1, 512, h/8, w/8]       resnet18: [-1,128, h/8, h/8]
        s4 = self.layer3(s3)    ## [-1, 1024, h/16, w/16]    resnet18: [-1,256, h/16, h/16]

        b1 = self.b1(s4)  # resnet18: [b_size, 64, h/16 , h/16]
        # b2 = self.b2(s4)
        # b3 = torch.cat([b1, b2], axis=1)

        d1 = self.d1(b1, s3)  # (64,128)
        d2 = self.d2(d1, s2)  # (256, 64)
        d3 = self.d3(d2, s1)  # (128, 64)
        d4 = self.d4(d3, s0)  # (64, 3)
